...
import Notescalationassignee from "./Notescalationassignee";
import Notinboundassignee from "./Notinboundassignee";
import Notinflowassignee from "./Notinflowassignee";
import Communication from "./Communication";
import Notcommassignee from "./Notcommassignee";
import Clock from "./Clock";
import Barassigneeesc from "./Barassigneeesc";
...
const data = [

  {
      ...
      notifcation: <Notescalationassignee />,
...

{
  ...
  notifcation: <Notinboundassignee />,
...
componentDidMount() {
    ...
    axios.get(`http://localhost:8080/api/countescalation/${this.state.user}` )
        ...
        axios.get(`http://localhost:8080/api/assigneename/${this.state.user}` )
    ...
    return (
      <div className="Home">
        ...
        {this.state.open === "Escalations" ? (
          <EscalationAssigneeComponent setOpen={this.setOpen} />
        ) : null}
        {this.state.open === "Inbound" ? (
          <FreshInboundAssignee setOpen={this.setOpen} />
        ) : null}
        {this.state.open === "Inflow" ? (
          <FreshInflowAssignee setOpen={this.setOpen} />
        ) : null}
        {this.state.open === "Communication" ? (
          <Communication setOpen={this.setOpen} />
        ) : null}
      </div>
    );
  }
